...
# loop to ask user for name, email, address, phone
...
# ask user for person to look up in dictionary persons:
    pname= input( 'enter the name of the person to find:')
    pdata = persons[pname] # what if the name is not found?
        #python "throws" an Exception - error message
    try:
        pdata = persons[pname]
    except: # if name not found catch the error here
        # not found should we add it?
        addnew = input(pname+' not found. Add as mew (Y/N)?')
        if addnew in 'Yy': #add new person
            persons[pname] = [] # empty dict entry
        else:
